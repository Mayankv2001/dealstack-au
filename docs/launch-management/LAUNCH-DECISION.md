...
| 2026-07-11 | CONDITIONALLY READY | **Migrations 009–012 reviewed and applied to prod on user authorisation ("finish it")**, plus follow-up 013 (revokes anon/authenticated EXECUTE on the two new SECURITY DEFINER trigger functions — clears all four advisor WARNs the new lints raised; only pre-existing INFO lints + the leaked-password dashboard toggle remain). Applying 009 also un-broke live `/cards`: the deployed `5b6d01d` code selected `review_by_date`/`bonus_stages` columns that did not yet exist, so the page had regressed to its empty state. Verified post-apply: `verify:schema` 18/18 tables, strict prod smoke **25/25**, nonce-based report-only CSP serving, `/api/health/data` 401 without auth. **Ongoing-offer payoff delivered:** NAB, Westpac and ANZ (confirmed, null expiry, review_by_date 2026-08-09, staged bonuses) published via triple-guarded SQL + explicit audit rows — live `/cards` now shows 4 offers; CBA stays archived. Fixed the one red CI spec from `5b6d01d` (mobile-viewport overflow on /resources — Stores nav link now hidden below `sm`, same as the /deals header); full e2e 21 passed / 1 skipped. |
