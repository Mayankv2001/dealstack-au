...
| OPS-7 | **Post-cleanup strict smoke re-run** against prod after OPS-2, to refresh the evidence: `npm run smoke -- --strict-content --base-url=https://dealstack-au.vercel.app`. (Passed 28/28 on 2026-07-10 before the final data cleanup; re-run is confirmation, not discovery.) | Mayank or manager | READY (after OPS-2) | 28/28 pass recorded in LAUNCH-DECISION. |
...
| [TASK-003](tasks/TASK-003-deals-disclaimer-wording-accuracy.md) | `/deals` disclaimer wording: "cached examples" understates real curated data | Low | Recommended | Confirmed | Small | Claude Sonnet (actual; Haiku recommended) | IMPLEMENTED | — | `6845117` | pending |

**TASK-001 follow-up (human-authorised production step, not gating):** apply migration 008 to prod (SQL editor / `supabase db push`), then re-run the Supabase security advisors (the `function_search_path_mutable` WARN should clear) and confirm `pg_proc.proconfig` shows `search_path=""` for `set_updated_at`. Rollback: `ALTER FUNCTION public.set_updated_at() RESET search_path;`. Recorded in FINAL-LAUNCH-CHECKLIST §3 and REVIEW-TASK-001.
