...
export async function sendMagicLink(formData: FormData) {
  ...
  const { error } = await supabase.auth.signInWithOtp({
    ...
    options: {
      ...
      // Admin identities are provisioned deliberately in Supabase. A public
      // login form must never create auth users or send mail to arbitrary new
      // addresses; the private admins table remains the authorization gate.
