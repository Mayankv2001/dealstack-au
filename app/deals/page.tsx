...
function DealGrid({ items, stores, now }: { items: DealListItem[]; stores: Awaited<ReturnType<typeof loadDealsBundle>>["stores"]; now: Date }) {
  return <div className="divide-y border-y">{items.map((item) => item.type === "deal" ? <DealCard key={item.deal.id} deal={item.deal} stores={stores} now={now} list /> : <div key={`group:${item.group.productGroup}`} className="py-4"><DealGroupCard group={item.group} now={now} /></div>)}</div>;
}
