...
import {
  ...
  releaseMonitorLock,
  type MonitorLock,
} from "@/lib/admin/repos/monitorLock";
 ...
 *   5. Acquire the DB lease           → duplicate/overlapping runs return safely.
 *   6. All gates pass                 → runMonitor() in WRITE mode, writing ONLY
 ...
 *
 * This is the only HTTP request path allowed to fetch, and it is secret-gated and
...
export async function GET(request: Request): Promise<Response> {
  ...
  // Vercel can deliver the same cron event more than once or overlap runs.
  // A short DB lease makes the write path single-flight across instances.
  let lock: MonitorLock | null;
  try {
    lock = await acquireMonitorLock();
  } catch (err) {
    return Response.json(
      { ok: false, ran: false, error: `Monitor lock failed: ${errMessage(err)}` },
      { status: 500 }
    );
  }
  if (!lock) {
    return Response.json({
      ...
      ran: false,
      alreadyRunning: true,
      note: "Another monitor invocation currently holds the run lock.",
    });
  }
  ...
  try {
    ...
    try {
      ...
      console.error(`[monitor] lock release failed: ${errMessage(err)}`);
    }
  }
}
