...
import { extractOffer, EXTRACTOR_VERSION, type ExtractedOffer } from "./extractOffer";
import { parseGcdbFeed, type GcdbFeedItem } from "./parseGcdbFeed";
...
export interface RunIngestDeps {
  ...
  insertRawItem(sourceId: string, item: GcdbFeedItem, contentHash: string, extraction: ExtractedOffer, now: Date): Promise<string>;
  /** Update a changed raw item in place (content, hash, extraction, last_seen). */
  updateRawItem(id: string, item: GcdbFeedItem, contentHash: string, extraction: ExtractedOffer, now: Date): Promise<void>;
...
export async function runGiftCardIngest(
  ...
  for (const item of items) {
    try {
      const extraction = extractOffer(item);
      const hash = contentHashOf(item);
      const before = byExternalId.get(item.externalId);

      if (!before) {
        const rawItemId = await deps.insertRawItem(source.id, item, hash, extraction, now);
      ...
      await deps.updateRawItem(before.id, item, hash, extraction, now);
      metrics.itemsUpdated++;
      const change = before.extraction
        ? classifyOfferChange(before.extraction, extraction)
        : null;
      const linkedToApproved = before.approvedOfferId != null;
      const needsCandidate =
        before.openCandidateId != null || // keep the open candidate current
        !linkedToApproved || // unreviewed items always re-stage
        (change?.requiresReview ?? true); // approved + material → back to review
      if (needsCandidate) {
        const changed = linkedToApproved || before.openCandidateId == null
          ? linkedToApproved
          : false;
        await deps.stageCandidate(source.id, {
          rawItemId: before.id,
          ...
          reviewStatus: changed ? "changed" : "new",
        });
        if (changed) metrics.candidatesChanged++;
