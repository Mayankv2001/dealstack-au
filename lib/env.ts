...
// ── Numeric env helpers (SERVER/SCRIPT ONLY) ─────────────────────────────────
// Lazy reads (functions) so importing this module never throws; each pipeline
// calls them on its own code path, never from a request-handling page or
// public route.
...
// Both the env master switch AND the gift_card_sources DB row must be enabled
