...
A deal-stacking platform for Australian shoppers that combines cashback portals, gift cards, and points programmes into a single research tool. Built as a full-stack portfolio project demonstrating Next.js 16, Supabase, and responsible data-pipeline design.
...
- Gift-card (GCDB) RSS ingestion is **gated and off by default** — requires `GCDB_INGEST_ENABLED=true` plus an enabled `gift_card_sources` row
- Ingested items are **staged for mandatory admin review** and never auto-published
- RSS/Atom parsing only — no HTML scraping
...
app/                      Next.js 16 App Router
  ...
  api/cron/               Secret-gated cron routes (never called client-side)
...
lib/
  ...
  feeds/                  RSS feed fetcher — pure, tested, no DB side-effects
  giftcards/              Gift-card ingest, valuation and presentation logic
...
tests/
  ...
  feeds/                  Vitest tests for the RSS fetcher and retry parsing
...
**Key safety boundary:** the Supabase service-role key is only used inside `lib/admin/repos/` and the cron API routes — never in client components or public routes.
...
Hosted on **Vercel (Hobby plan)**. One cron job per day is the Hobby plan maximum — `vercel.json` schedules `GET /api/cron/daily-cleanup` daily. It unpublishes expired offers and archives card offers past their review date, and makes no outbound requests.

The gift-card ingest, lifecycle and reconcile jobs run from **GitHub Actions**, calling their secret routes with `Authorization: Bearer $CRON_SECRET`. See [docs/gift-card-pipeline.md](docs/gift-card-pipeline.md).
...
| `CRON_SECRET` | Bearer token for every cron route |
...
| `GCDB_INGEST_ENABLED` | Optional — only to activate gift-card ingestion; default off |
...
- [ ] Enable `GCDB_INGEST_ENABLED=true` in Vercel env if gift-card ingestion is wanted
- [ ] Review any staged gift-card candidates in `/admin/gift-cards` before publishing
