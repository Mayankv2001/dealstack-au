...
describe("expiryIntegrityBoundary", () => {
  // Offers expire at Sydney midnight; the cleanup cron fires later that day.
...
describe("summarisePublishedDataHealth", () => {
  ...
  it("sums both maps independently across every offer type", () => {
    ...
    expect(health.totalOverdue).toBe(17);
    expect(health.totalExpiredStillPublished).toBe(18);
    expect(health.ok).toBe(false);
