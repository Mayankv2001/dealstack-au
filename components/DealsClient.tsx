...
import { formatWeekLabel } from "@/lib/offers/availability";
...
function isExpiringSoon(expiry: string | null, asOfDate: string): boolean {
  if (!expiry) return false;
  const diff =
    new Date(`${expiry}T12:00:00Z`).getTime() -
...
function buildGiftCardDeals(
  ...
  return offers.map((o): GiftTaggedDeal => {
    ...
    const soon = isExpiringSoon(o.expiryDate, asOfDate);
...
function buildCashbackDeals(
  ...
  return offers.map((o): TaggedDeal => {
    ...
    if (isExpiringSoon(o.expiryDate, asOfDate)) tags.add("expiring-soon");
    return {
      tags,
      data: {
        ...
        expiryDate: o.expiryDate,
        expiringSoon: isExpiringSoon(o.expiryDate, asOfDate),
        lastCheckedAt: o.lastCheckedAt,
        confidence: o.confidence,
        citations: o.citations,
      },
...
function buildSignalDeals(
  ...
  return offers
    .map((o): SignalTaggedDeal => {
      ...
      const soon = isExpiringSoon(o.expiryDate ?? null, asOfDate);
      if (soon) tags.add("expiring-soon");
      return {
        tags,
...
export default function DealsClient({
  ...
  const giftCardDeals = useMemo(
    () => buildGiftCardDeals(giftCardOffers, asOfDate),
    [giftCardOffers, asOfDate]
  );
  const pointsDeals = useMemo(
    () => buildPointsDeals(pointsOffers, nameOf, asOfDate),
    [pointsOffers, nameOf, asOfDate]
  );
  const cashbackDeals = useMemo(
    () => buildCashbackDeals(cashbackOffers, nameOf, asOfDate),
    [cashbackOffers, nameOf, asOfDate]
  );
  const signalDeals = useMemo(
    () => buildSignalDeals(ozBargainSignals, nameOf, asOfDate),
    [ozBargainSignals, nameOf, asOfDate]
  );
          ...
          {topStacks.length > 0 ? (
            ...
            <Card className="py-0 shadow-sm">
              <CardContent className="py-8 text-center">
                <p className="font-medium">No verified stacks are live right now</p>
                <p className="mt-1 text-sm text-muted-foreground">
                  Check back after the next review. DealStack will not substitute
