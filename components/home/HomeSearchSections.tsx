...
export function HomeSearchSections({
  ...
  return (
    <>
        ...
        <div className="page-container py-10 sm:py-14 lg:py-16">
          <div className="max-w-4xl">
            ...
            <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-sm font-semibold">
              ...
              >
                Browse today’s deals <ArrowRight aria-hidden className="size-4" />
              </Link>
              <a href="#stores" className="text-muted-foreground hover:text-foreground">
                Search by store
              </a>
            </div>
            <div className="mt-6 flex flex-wrap gap-3 text-xs font-semibold text-muted-foreground">
              {["Human-reviewed", "Sources visible", "Points kept separate from cash"].map((item) => (
                <span key={item} className="inline-flex items-center gap-1.5">
                  <Check aria-hidden className="size-3.5 text-emerald-700" /> {item}
                </span>
              ))}
            </div>
          </div>
      ...
      <section className="page-container py-8 sm:py-10" aria-labelledby="featured-plan-heading">
        <p className="eyebrow">Featured purchase plan</p>
        <h2 id="featured-plan-heading" className="mt-2 text-2xl font-black tracking-tight">
        ...
        {heroStack ? (
          ...
          <Card className="mt-5 border-dashed bg-background shadow-none">
            <CardContent className="p-6 text-sm text-muted-foreground">
              No current stack is ready to feature. Search a store to review its available saving layers.
            </CardContent>
          </Card>
        )}
      </section>
      ...
      <section className="page-container py-8 sm:py-10" aria-labelledby="quick-start-heading">
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="eyebrow">Choose your path</p>
            <h2 id="quick-start-heading" className="mt-2 text-xl font-black tracking-tight sm:text-2xl">
              What do you want to do?
            </h2>
          </div>
          <Link href="/search" className="hidden items-center gap-1 text-sm font-bold text-emerald-700 hover:underline sm:inline-flex">
      ...
      <section id="stores" className="page-container scroll-mt-24 py-10 sm:py-14">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="eyebrow">Search by store</p>
            <h2 className="section-title mt-2">Start with where you’re shopping</h2>
          </div>
          {needle ? (
...
function HeroReceipt({ recommendation }: { recommendation: StackRecommendation }) {
  ...
  const steps = buildStackSteps(recommendation.merchantName, recommendation).filter(
    (step) => !step.chooseOne
  );
      ...
      <CardContent className="p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b pb-4">
          ...
          <span className="rounded-full border border-emerald-500/25 bg-emerald-500/10 px-2 py-1 text-[10px] font-semibold text-emerald-800 dark:text-emerald-300">
            Plan confidence: {recommendation.confidence === "confirmed" ? "High" : "Medium"}
          </span>
        </div>
        ...
        <div className="mt-5 grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
          <div>
            <dl className="divide-y rounded-lg border px-3 text-sm tabular-nums">
              <ReceiptLine label="Original cart" value={formatAUD(outcome.originalCart)} />
              <ReceiptLine label="Pay at checkout" value={formatAUD(outcome.cashPaidForCheckout)} />
              {outcome.cashbackLater > 0 ? (
                <ReceiptLine label="Cashback expected later" value={formatAUD(outcome.cashbackLater)} saving />
              ) : null}
              {outcome.pointsEarned > 0 ? (
                <ReceiptLine label="Points earned" value={`~${outcome.pointsEarned.toLocaleString("en-AU")}`} />
              ) : null}
            </dl>
            <div className="mt-4 flex items-end justify-between gap-3 border-t pt-4">
              <div>
                <p className="text-xs text-muted-foreground">Estimated effective cost</p>
              ...
              <div className="text-right">
                <p className="font-bold">Save {formatAUD(recommendation.totalSaving)}</p>
                <p className="text-xs font-semibold text-muted-foreground">{recommendation.effectiveDiscountPercent}% cash value</p>
              </div>
            </div>
          </div>

          <div>
            <h4 className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Use this order</h4>
        ...
        <Link href={`/stores/${recommendation.merchantId}`} className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-emerald-800 hover:underline dark:text-emerald-300">
          View {recommendation.merchantName}’s full plan <ArrowRight aria-hidden className="size-4" />
        </Link>
...
function ReceiptLine({ label, value, saving = false }: { label: string; value: string; saving?: boolean }) {
  return (
    <div className="flex items-start justify-between gap-4 py-2.5">
      ...
      <dd className={saving ? "shrink-0 font-semibold text-emerald-800 dark:text-emerald-300" : "shrink-0 font-semibold"}>{value}</dd>
    </div>
  );
}
