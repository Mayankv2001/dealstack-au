...
const NAV_LINKS = [
  ...
  { label: "Cashback", href: "/deals?view=cashback", matchPath: "/cashback", icon: CircleDollarSign },
      ...
      <div className="mx-auto flex h-[4.5rem] max-w-7xl items-center gap-3 px-4 sm:px-6">
        ...
        >
          {NAV_LINKS.map(({ label, href, ...link }) => {
            const active = isActive(pathname, "matchPath" in link ? link.matchPath : href);
            return (
              <Link
                key={href}
                href={href}
      ...
      >
        {MOBILE_DOCK_LINKS.map(({ label, href, icon: Icon, ...item }) => {
          ...
          return (
            ...
            >
              <span className={cn("flex size-7 items-center justify-center rounded-full", primary && "-mt-5 size-12 border-4 border-background bg-emerald-700 text-white shadow-lg")}>
